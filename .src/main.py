...
if __name__ == '__main__':
    ...
    if not args.action:
        if not args.excelpath:
            for root, dirs, files in os.walk(ALL_EXCEL_PATH):
                ...
                for file in files:
                    ...
                    new_files, updated_files = py_git.get_changed_files(path)
                    if updated_files:
                        ...
                        new_files, updated_files = py_git.get_changed_files(text_path)
        ...
        if args.action=='to_text' and args.excelpath:
            new_files, updated_files = py_git.get_changed_files(args.excelpath)
            if updated_files:
