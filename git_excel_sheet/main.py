...
@click.option(
    "--action",
    type=click.Choice(['to_text', 'to_excel', '']),
    default='',
...
def main(action, wait, excelpath):
    ...
    if not action:
        ...
        if action=='to_text' and excelpath:
            new_files, updated_files = get_changed_files(excelpath)
            print(new_files, updated_files)
            if updated_files:
                excel_paths_to_parse.append(excelpath)
            elif new_files:
                print("No action! New file is parsing at commit time")
        elif action=='to_excel' and excelpath:
